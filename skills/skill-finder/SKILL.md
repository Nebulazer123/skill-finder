...
Find, vet, compare, improve, and recommend installable skills or task capabilities. "Skill" includes agent skills, MCP servers, apps/connectors, workflows, CLIs, packages, validators, docs, Hugging Face models/datasets/Spaces, and composed bundles. Do not rely on private/global AGENTS routing. Skill Finder requires its core discovery stack before a real run: Agent Skills CLI or skills.sh access, GitHub MCP, DeepWiki MCP, Context7, Browserbase Browse CLI, `rg`, `git`, and `python3`. If required setup is missing, stop before ranking, explain what is missing, and ask whether to install or configure it. Recommend only capabilities usable by an AI agent; human-only sites are source or method evidence unless they expose an API, CLI, connector, workflow, package, MCP server, model, dataset, Space, or skill.

## Workflow

1. Infer the real skill need from the request, repo, long context, or blocker.
2. Run the required setup gate before searching. Verify Agent Skills CLI or skills.sh access, GitHub MCP, DeepWiki MCP, Context7, Browserbase Browse CLI, `rg`, `git`, and `python3` with harmless checks. If any required route is missing or not callable, stop and output a Required Setup Block instead of running a weakened recommendation.
